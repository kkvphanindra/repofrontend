...
const VerifyAccount = ({route, navigation}) => {
  ...
  let loginNum = number.slice(-10);
  console.log(number, loginNum, 'getConfirm');
  const confirmCode = async () => {
    console.log('confirmCode value ==>', code);
    try {
      await getConfirm.confirm(code);
      console.log('success.', loginNum);
      setModalVisible(false);
      dispatch(login(loginNum))
    ...
    <View style={styles.container}>
      <View style={styles.wrapper}>
        ...
        <Text style={styles.subContent}>
          Enter 6-digit code we have sent to {number}
        </Text>
        <View style={styles.OTPWrapper}>
          <OTPInput maximumLength={6} onChange={onChangeText} />
        </View>
      ...
      <Pressable onPress={confirmCode} style={styles.buttonContainer}>
        <LinearGradient
